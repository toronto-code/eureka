...
from sqlalchemy import JSON, DateTime, Float, ForeignKey, Index, String, Text, Integer
...
except ImportError:  # pragma: no cover — only missing in dev before pip install
    Vector = None
    _VECTOR_AVAILABLE = False

# Dimension for all embedding columns. Change here → change in 0002_add_vectors.py → new migration.
EMBEDDING_DIM = 1536
...
class EventRow(Base):
    ...
    # Added by migration 0002. Nullable — populated lazily by knowledge service.
...
# ---------------------------------------------------------------------------
# document_embeddings — added by migration 0002
# Written by services/knowledge; read by apps/api for semantic search
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# agent_memories — added by migration 0004
# Sole writer: services/agent-runtime. Used for long-term agent recall.
# ---------------------------------------------------------------------------
...
class AgentMemoryRow(Base):
    ...
    task_id: Mapped[str | None] = mapped_column(
        String, ForeignKey("agent_tasks.task_id", ondelete="SET NULL"), nullable=True, index=True
    )
    correlation_id: Mapped[str] = mapped_column(String, index=True)
    memory_type: Mapped[str] = mapped_column(String, default="fact", index=True)
    content: Mapped[str] = mapped_column(Text, nullable=False)
